        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-add-label">Create Data {{ $parents->name }}</h5>
        ...
        <div class="grid"
            ...
            @foreach ($parents->childs as $child)
                <div class="grid-item" style="border: none; background: none; box-shadow: none;">
                    <img src="{{ asset($child->thumbnail) }}" alt="{{ $child->name }}"
                        style="width: 100%; max-height: 150px; object-fit: contain; margin-bottom: 16px; cursor: pointer;"
                        onmouseenter="showSmallModal('{{ asset($child->thumbnail) }}', '{{ $child->name }}', '{{ $child->description }}', this)"
                        onmouseleave="hideSmallModal()"
                        onclick="window.location.href='{{ route('katalog.detail', ['parentId' => $parents->id, 'childId' => $child->id]) }}';">
                </div>
            @endforeach
        </div>
    @endif
