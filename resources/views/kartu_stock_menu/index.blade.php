        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <button type="button" class="btn btn-alt-primary  btn-sm" data-bs-toggle="modal"
                    ...
                    <i class="fa fa-plus text-info me-1"></i>{{ $data->nama }} Baru</button>
            </div>
    ...
    <div class="modal fade" id="modalCreateUpdate" tabindex="-1" role="dialog" aria-labelledby="modal-block-fadein"
        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    <div class="block-header block-header-default">
                        <h3 class="block-title">Inputan {{ $data->nama }}</h3>
                        <div class="block-options">
                            <button type="button" class="btn-block-option" data-bs-dismiss="modal" aria-label="Close">
                                <i class="fa fa-fw fa-times"></i>
                            </button>
                    ...
                    <div class="block-content fs-sm">
                        <form id="createForm">
                            <div class="form-group mb-4">
                                <label for="qty_stock">Stock</label>
                                <input required="" autocomplete="off" type="number" class="form-control mt-2"
                                    id="qty_stock" name="qty_stock" placeholder="Masukkan Stock">
                            </div>
                            <div class="form-group mb-4">
                                {{-- <input required="" autocomplete="off" type="number" class="form-control" id="qty_stock"
                                    name="type" placeholder="Stock"> --}}
                                <label for="">Tipe</label>
                                <div class="d-flex mt-2 align-items-center" style="gap: 20px">
                                    <div class="form-check" style="font-size: 18px">
                                        <input class="form-check-input" type="radio" name="type" value="1"
                                            id="in_type" checked>
                                        <label class="form-check-label" style="color: green; font-weight: bold" for="in_type">
                                            Masuk
                                        </label>
                                    </div>
                                    <div class="form-check" style="font-size: 18px">
                                        <input class="form-check-input" type="radio" name="type" value="2"
                                            id="out_type">
                                        <label class="form-check-label" style="color: red; font-weight: bold" for="out_type">
                                            Keluar
                                        </label>
                                    </div>
                                </div>
...
@section('js_after')
    ...
    <script>
        ...
        const table = $("#dataTable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('kartu-stock.menu.dataTable', $data->id) }}",
                method: "POST",
                data: function(d) {
                    d.filter = filter;
                    return d;
            ...
            columns: [
                ...
                },
                {
                    name: "created_at",
                    data: (item) => {
                        return `<span style="white-space: nowrap">${moment(item?.created_at).format('HH:mm:ss')}</span>`;
                    },
                },
                {
                ...
                },
                {
                    ...
                    data: (item) => {
                        ...
                        `<div class="badge bg-success" style="font-weight: bold; font-size: 14px">${item?.qty_stock}</div>`
                        : '';
                    }
                },
                {
                    ...
                    data: (item) => {
                        ...
                        `<div class="badge bg-danger" style="font-weight: bold; font-size: 14px">${item?.qty_stock}</div>`
                        : '';
                    }
                },
                {
                    name: 'qty_saldo',
                    data: (item) => {
                        return `<div class="badge bg-black" style="font-weight: bold; font-size: 14px">${item?.qty_saldo}</div>`
                    }
                },
                {
                    name: "action",
                    className: "sticky-table",
                    data: ({action, DT_RowIndex}) => {
                        if(DT_RowIndex === 1) {
                            return action;
                        }
                        return '';
        ...
        $('#submitHandle').on('submit', function(e) {
            e.preventDefault();
            let url = `{{ route('kartu-stock.menu.destroy', ':id') }}`;
        ...
        $("#createForm").on("submit", function(e) {
            ...
            $.ajax({
                url: "{{ route('kartu-stock.menu.store', $data->id) }}",
                method: "POST",
                data: $(this).serialize(),
        ...
        $('#modalCreateUpdate').on('hidden.bs.modal', function() {
            ...
            modal.find('.modal-title').text('Inputan {{ $data->nama }}');
