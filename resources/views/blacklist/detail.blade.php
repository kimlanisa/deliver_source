...
@section('js_after')
    ...
    <script>
        //DELETE
        $('body').on("click", ".btn-delete-blacklist", function() {
            var id = $(this).attr("id");
            $(".btn-destroy-blacklist").attr("id", id);

            $("#modal-delete").modal("show");
        });

        $(".btn-destroy-blacklist").on("click", function() {
            var id = $(this).attr("id")
            console.log(id);
            $.ajax({
                url: `{{ url('/blacklist/destroyResi/${id}') }}`,
                method: 'DELETE',
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-4">
                <div class="flex-grow-1">
                    ...
                    <h2 class="fs-base lh-base fw-medium text-muted mb-0">
                        No Blacklist : <span
                            class="fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-info-light text-info fs-sm">{{ $blacklist->no_blacklist }}</span>
                    </h2>
                </div>
    ...
    <div class="content">
        <div class="block block-rounded">
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="javascript:void(0)" id="{{ $dt->no_resi }}"
                                                class="btn btn-sm btn-alt-secondary js-bs-tooltip-enabled btn-delete-blacklist"
            ...
            <p class="alert alert-info fs-sm">
                <i class="fa fa-fw fa-info me-1"></i> <b>Catatan : </b> {{ $blacklist->catatan }}
            </p>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="block block-rounded block-transparent mb-0">
                    ...
                    <div class="block-content block-content-full text-end bg-body">
                        ...
                        <button type="button" class="btn btn-sm btn-danger btn-destroy-blacklist">Hapus</button>
                    </div>
                </div>
            </div>
        </div>
