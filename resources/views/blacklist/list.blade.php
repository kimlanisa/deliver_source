    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                @if (canPermission('Daftar Blacklist.Create'))
                <a href="{{ route('blacklist.create') }}" class="btn btn-sm btn-alt-secondary"><i
                        class="fa fa-plus text-info me-1"></i>Blacklist Baru</a>
                @endif
            </div>
                    ...
                    <table class="table table-bordered table-striped table-vcenter fs-sm" id="dataTable">
                        <thead>
                            <tr class="text-center">
                                ...
                                <th style="width: 200px;">NAMA EKSPEDISI</th>
                                <th>WAKTU DISCAN</th>
...
@section('js_after')
    ...
    <script>
        ...
        const table = $("#dataTable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: `{{ route('blacklist.getDataBlacklist') }}`,
                method: "POST",
                data: function(d) {
                    d.filter = filter;
                    return d;
            ...
            columns: [{
                ...
                {
                    name: "ekspedisi.expedisi",
                    data: ({
                        ekspedisi
                    }) => {
                        return `<span class="badge py-1 px-2 text-white" style="background: ${ekspedisi.color}; font-size: 13px">${ekspedisi.expedisi}</span>`
                    }
                },
                {
                    name: "created_at",
        ...
        $('#submitHandle').on('submit', function(e) {
            e.preventDefault();
            let url = `{{ route('blacklist.destroy', ':id') }}`;
